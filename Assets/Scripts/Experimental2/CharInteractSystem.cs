...
namespace CL03
{
    public class CharInteractSystem : MonoBehaviour
    {
        InputHandler input;
        CharManager2D character;
        CharInventorySystem inventory;
        Physics2D physics;
		...
		public Vector2 objectColliderSize;// Start is called before the first frame update
		...
		void Awake()
		{
			...
			inventory = GetComponent<CharInventorySystem>();
			...
			interactablesLayer |= character.staticInteractablesLayer; //interactables in environment
		}
		...
		private void FixedUpdate()
		{
			...
			if (input.interactPressed && !interactCoolingDown)
			{
				...
				if (ItemsCheck())
				{
					if (character.isSelected && !interactCoolingDown)
					{
						InteractCheck();
					}
				}
	...
	//			inventory.objectBeingHeld.GetComponent<HoldableObjects>().Interact(character);
		...
		void InteractCheck()
		{
			...
			{
				print("Interact Check true");
	//			WithInArmsReach.GetComponent<InteractableObjects>().Interact(character);
			}
