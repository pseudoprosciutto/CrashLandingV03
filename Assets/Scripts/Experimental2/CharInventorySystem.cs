...
namespace CL03
{
    public class CharInventorySystem : MonoBehaviour
    {
		...
		private void PositionOfItemInHands()
		{
			...
			if (input.vertical > .2f
			//	&& !character.isHeadBlocked && !character.ObjectInFrontCornerCheck() && !(character.hitOverHeadLeft || character.hitOverHeadRight) && !character.ObjectChangeHitFrontCheck()
				)
			{ isHoldingSomethingAbove = true; }
			//engine.ChangeCollider(objectCollider.size,true); }
			if (input.vertical < -.2f
				//&& !character.ObjectInFrontCornerCheck() && !(character.hitOverHeadLeft || character.hitOverHeadRight) && !character.ObjectChangeHitFrontCheck()
                )
			{ isHoldingSomethingAbove = false; }
		}
